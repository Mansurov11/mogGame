...
export default function WordSnake() {
  ...
  // gameState drives ALL UI rendering — always kept in sync
  const [gameState, setGameState] = useState(null);
  const [particles, setParticles] = useState([]);
  const [speedLevel, setSpeedLevel] = useState(1);
  // wordComplete: null | { nextWord, bonusScore }
  const [wordComplete, setWordComplete] = useState(null);

  const canvasRef = useRef(null);
  const gameRef = useRef(null); // mutable game object for the loop
  ...
  // ── sync gameRef → gameState so UI always reflects latest ──────
  ...
  const drawGame = useCallback(
    (g) => {
      ...
      for (let i = 1; i < n; i++) {
        ...
        while (d <= len) {
          const fx = pts[i - 1].x + (dx / len) * d,
            fy = pts[i - 1].y + (dy / len) * d;
          const px2 = -dy / len,
            py2 = dx / len;
          ctx.beginPath();
      ...
      const ecx = hx + g.dir.x * TUBE_R * 0.55,
        ecy = hy + g.dir.y * TUBE_R * 0.55;
      [1, -1].forEach((s) => {
        const ex = ecx + perp.x * s * TUBE_R * 0.42,
          ey = ecy + perp.y * s * TUBE_R * 0.42;
        ctx.fillStyle = "#fff";
        ctx.beginPath();
        ctx.arc(ex, ey, TUBE_R * 0.28, 0, Math.PI * 2);
        ctx.fill();
      ...
      const tongueOut = Math.abs(Math.sin(Date.now() / 200)) > 0.5;
      if (tongueOut) {
        const tx = hx + g.dir.x * (TUBE_R + 6),
          ty = hy + g.dir.y * (TUBE_R + 6);
        ...
        [1, -1].forEach((s) => {
          ...
          ctx.moveTo(tx, ty);
          ctx.lineTo(
            tx + g.dir.x * 5 + perp.x * s * 5,
            ty + g.dir.y * 5 + perp.y * s * 5,
          );
          ctx.stroke();
        });
      }
  ...
  useEffect(() => {
    ...
    const loop = (ts) => {
      ...
      // always draw so canvas stays fresh
      drawGame(g);

      // paused (word-complete popup) or dead → skip logic
      ...
      // collect letter
      const active = g.letters.find(
        (l) => l.active && l.x === head.x && l.y === head.y,
      );
      if (active) {
        ...
        if (g.letters.length === 0) {
          ...
          // update state THEN show popup
          setGameState({ ...g });
          setWordComplete({ nextWord, bonus });
          // pre-load next word into gameRef so it's ready when player continues
          g.word = nextWord;
          g.collected = [];
          ...
          return; // skip syncState below — already called setGameState
        }
      }

      // ── sync to React state every tick so UI updates ────────────
      setGameState({ ...g });
  ...
  }, [screen, skin, spawnParticles, drawGame]);
  ...
  if (screen === "game") {
    ...
    return (
      <div
        className={`min-h-screen bg-linear-to-br ${sk.bg} flex flex-col items-center justify-center select-none`}
      >
        <div className="w-full max-w-2xl px-2">
          {/* Header */}
          <div className="flex items-center justify-between mb-3 px-1">
            <span className="text-white font-black text-2xl flex items-center gap-2">
              <Snail size={24} className={sk.tag} /> Word
              <span className={sk.tag}>Snake</span>
            </span>
            <div className="flex items-center gap-4">
              <div className="text-center">
                ...
                <div className={`text-white font-black text-xl ${sk.tag}`}>
                  {g2?.score ?? 0}
                </div>
              </div>
              <div className="text-center">
                ...
                <div className={`font-black text-xl ${sk.tag}`}>
                  ×{speedLevel}
                </div>
              </div>
              <button
          ...
          {/* Word progress — always rendered from gameState */}
          {g2 && (
            <div
              className={`bg-gray-900 rounded-xl p-3 mb-3 border ${sk.border} border-opacity-30`}
            >
                ...
                <div className="flex gap-1 flex-wrap">
                  {g2.word.split("").map((l, i) => {
                    ...
                    return (
                      <span
                        key={i}
                        className="w-8 h-8 flex items-center justify-center rounded-lg font-black text-base transition-all duration-300"
                        style={{
                          background: done
                            ? sk.head
                            : next
              ...
              <div className="flex justify-between mt-1">
                <span className="text-gray-500 text-xs">
                  {g2.collected.length}/{g2.word.length} letters
                </span>
                <span className="text-gray-500 text-xs">
                  {g2.wordsCompleted ?? 0} words done
                </span>
              </div>
            </div>
          )}
          ...
          <div
            ...
            style={{ border: `10px solid ${sk.head}44` }}
          >
            <canvas
              ref={canvasRef}
              width={W}
            ...
            {particles.map((p) =>
              p.letter ? (
                <div
                  key={p.id}
                  className="absolute pointer-events-none font-black  transition-all duration-600"
                  style={{
                    left: p.x,
                    top: p.y,
                    color: p.color,
            ...
            {wordComplete && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm">
                <div
                  className={`bg-gray-900 border-2 ${sk.border} rounded-2xl p-8 text-center shadow-2xl mx-4 animate-in`}
                >
                  ...
                  <p className="text-gray-400 mb-1">
                    You spelled{" "}
                    <span className={`font-black ${sk.tag} text-lg`}>
                      {g2?.word}
                    </span>
                  </p>
                  <p className={`${sk.tag} font-bold text-lg mb-4`}>
                    +{wordComplete.bonus} bonus points!
                  </p>
                  <div className="bg-gray-800 rounded-xl p-4 mb-6 border border-gray-700">
                    ...
                    <div className="flex gap-2 justify-center flex-wrap">
                      {wordComplete.nextWord.split("").map((l, i) => (
                        <span
                          key={i}
                          className="w-9 h-9 flex items-center justify-center rounded-lg font-black text-lg text-white border border-gray-600"
                  ...
                  <button
                    ...
                    className={`${sk.btn} text-white font-black px-10 py-3 rounded-xl shadow-lg transition-all active:scale-95 flex items-center gap-2 mx-auto`}
                  >
            ...
            {g2 && !g2.alive && !wordComplete && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm">
                <div
                  className={`bg-gray-900 border-2 ${sk.border} rounded-2xl p-8 text-center shadow-2xl`}
                >
                  ...
                  <p className="text-gray-400 mb-1">
                    You spelled:{" "}
                    <span className={`font-bold ${sk.tag}`}>
                      {g2.collected.join("") || "nothing"}
                    </span>
                  </p>
                  ...
                  <div className={`text-4xl font-black ${sk.tag} mb-6`}>
                    {g2.score} pts
                  </div>
                  <div className="flex gap-3 justify-center">
                    ...
                    <button
                      ...
                      className={`${sk.btn} text-white font-black px-8 py-3 rounded-xl shadow-lg transition-all active:scale-95`}
                    >
                      Menu
                    </button>
                  </div>
              ...
              <div key={ri} className="contents">
                {row.map((d, ci) =>
                  d ? (
                    <button
                      key={ci}
                      onPointerDown={() => handleDir(d.x, d.y)}
